...
def write_pred(pred_str, label_str, ids_path, path):
    ids = open(ids_path).readlines()
    d = defaultdict(dict)
    for id, s1, s2 in zip(ids, pred_str, label_str):
        id = id.rstrip()
        d[id]["pred"] = s1
        d[id]["label"] = s2
...
def build_compute_metrics_fn(task_name: str, tokenizer: PreTrainedTokenizer, output_dir: str, data_dir: str, mode: str = "val") -> Callable[[EvalPrediction], Dict]:
    def non_pad_len(tokens: np.ndarray) -> int:
        return np.count_nonzero(tokens != tokenizer.pad_token_id)

    def decode_pred(pred: EvalPrediction) -> Tuple[List[str], List[str], List[str], List[str]]:
        ...
        ids_path = Path(data_dir).joinpath(f"{mode}.ids")
        path = Path(output_dir).joinpath(f"{mode}_pred_label.json")
        write_pred(pred_str, label_str, ids_path, path)
        ...
        path = Path(output_dir).joinpath(f"{mode}_pred_label_subtask2.json")
        write_pred(pred_str_subtask2, label_str_subtask2, ids_path, path)

        return pred_str_subtask1, pred_str_subtask2, label_str_subtask1, label_str_subtask2

    def summarization_metrics(pred: EvalPrediction) -> Dict:
        pred_str, label_str = decode_pred(pred)
        rouge = calculate_rouge(pred_str, label_str)
        summ_len = np.round(np.mean(lmap(non_pad_len, pred.predictions)), 1)
        rouge.update({"gen_len": summ_len})
        return rouge

    def subtask1_metrics(p: EvalPrediction):
        metric = load_metric("./squad.py", cache_dir="./cache")
        return metric.compute(predictions=p.predictions, references=p.label_ids)

    def translation_metrics(pred: EvalPrediction) -> Dict:
        pred_str_subtask1, pred_str_subtask2, label_str_subtask1, label_str_subtask2 = decode_pred(pred)

        formatted_pred_subtask1 = [{"id": str(k), "prediction_text": v} for k, v in enumerate(pred_str_subtask1)]
        references_subtask1 = [{"id": str(k), "answers": {'answer_start': [0], 'text': [v]}} for k, v in enumerate(label_str_subtask1)]
        metric = subtask1_metrics(EvalPrediction(predictions=formatted_pred_subtask1, label_ids=references_subtask1))

        bleu = calculate_bleu(pred_str_subtask2, label_str_subtask2)
